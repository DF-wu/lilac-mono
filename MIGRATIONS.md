...
- `surface.telegram`: the Telegram surface. It is disabled by default, so adding it changes no existing behavior. Frozen v1 configs receive the same universal fallback (`enabled: false`) but cannot enable or configure the surface — enabling Telegram requires `configVersion: 2`. Keys: `enabled`, `token` (stored directly in `core-config.yaml`), `botName`, `botUsername`, `allowedChatIds`, `allowedUserIds`, `dbPath`, `outputMode`, `parseMode`, `streamEditIntervalMs`, `outputNotification`, `workingIndicators`, `commandMenu`, and `markdownTableRender`. The token is intentionally not read from the process environment. Existing v2 configs using `tokenEnv` are rejected with a migration error; copy the secret to `token` and restart Core.
