...
- `tools.generate.image.provider`: transport routing switch (`default` | `openai-compatible`). When set to `openai-compatible`, image generation calls route through the OpenAI-compatible provider configured via `OPENAI_COMPATIBLE_BASE_URL`; `OPENAI_COMPATIBLE_API_KEY` is optional for endpoints that do not require bearer authentication. Aliases and adapters remain unchanged; all aliases route together. Canonical model IDs are fixed. Selecting third-party routing has no automatic fallback. See `docs/generate-image-openai-compatible.md` for setup and usage.
