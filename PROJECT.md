...
- `tools.generate.image.models`: optional ordered default image model specs for `generate.image` (v2-only). Empty preserves built-in aliases; non-empty is an allowlist and default order. Specs must be a built-in alias (`nanobanana-2`, `nanobanana-pro`, `gpt-5-image`, `grok-imagine-image-pro`, `grok-imagine-image`, `nanobanana`) or `provider/model` with provider in `openai`, `openai-compatible`, `openrouter`, `xai`, `vercel`. Third-party OpenAI-compatible image APIs use `openai-compatible/<model-id>` plus `OPENAI_COMPATIBLE_BASE_URL` (required) and optional `OPENAI_COMPATIBLE_API_KEY`.
- `tools.generate.image.defaults` / `profiles`: optional global and per-model parameter defaults (`size`, `aspectRatio`, `maxRetries`, provider `options`) plus profile `useWhen` guidance surfaced in the tool description. Portable `seed` is intentionally unsupported because the pinned OpenAI-compatible image adapter does not forward it reliably.
- OpenAI-compatible image models often need provider-specific fields under `options`; for example `openai-compatible/gpt-image-2` may require a profile with `size: "1024x1024"` and `options.quality: "high"` depending on the upstream gateway. Shorthand options are namespaced automatically (`openaiCompatible`, `gateway` for Vercel).
- Runtime verification path: `tools --help generate.image` should show configured default models, explicit provider specs, and profile defaults; then `tools generate.image --output=json ...` should return selected `model`, output path, MIME type, and byte count. `Default models: none` is valid when `models` is empty but an explicit provider is configured and the caller must pass `provider/<model-id>`. If both default models and explicit specs are `none`, check `core-config.yaml` and provider env.
- Operator guide: `docs/generate-image-third-party-providers.md`.
