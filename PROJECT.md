...
the Discord adapter starts and warms one Chromium browser and page, serializes table renders on that
page, and closes it when disabled or disconnected. Debug logs in `surface:discord:table-image` cover
browser startup, warmup, render waits, layout, screenshots, and shutdown. Correlated
`surface:discord:output` logs time preview completion, final chunk preparation, and Discord sends.
The renderer keeps generated bytes in memory, uses measured column widths, and falls back to the Unicode text renderer on rendering failure or
when a PNG exceeds the 20 MiB upload limit. The image style also falls back to Unicode in other output
modes and streaming previews. Image cells support bold, italic, inline code, strikethrough, Discord
