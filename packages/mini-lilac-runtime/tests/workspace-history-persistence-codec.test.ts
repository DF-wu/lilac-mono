...
describe("workspace history persistence codecs", () => {
  it("decodes every current v1 record without changing the shipped format/index versions", () => {
    const decoded = [
      decodeWorkspaceHistoryOwnership({
      ...
      decodeWorkspaceHistoryRestorePlan({
        serialized: JSON.stringify({
          ...
          workspaceId,
          canonicalCwd,
          sourceRootTreeOid: oid,
          targetRootTreeOid: oid,
    ...
    expect(decoded.every((result) => result.status === "ok")).toBe(true);
    expect(
      decoded.map((result) => (result.status === "ok" ? result.value.provenance : "error")),
    ).toEqual(["current", "current", "current", "current", "current", "current"]);
  });
