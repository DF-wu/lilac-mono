...
if not attempt or not deadline then return {"panic", "state-malformed"} end
local now = now_ms()
...
redis.call("HSET", KEYS[2], "failure_kind", "handler-error", "failure_tag", ARGV[6], "failure_message", ARGV[7])
if attempt == 5 then
  redis.call("HSET", KEYS[2], "state", "claimed")
  return {"exhausted", tostring(attempt), ARGV[2], ARGV[3], tostring(deadline)}
end
local due = now + tonumber(ARGV[8])
redis.call("HSET", KEYS[2], "state", "retry-scheduled", "due_at", tostring(due))
redis.call("HDEL", KEYS[2], "token", "owner_id", "consumer_id", "lease_deadline")
