...
local completed = valid_completed_attempts(redis.call("HGET", KEYS[2], "attempt"))
local deadline = valid_deadline(redis.call("HGET", KEYS[2], "lease_deadline"))
if completed == nil or not deadline then return {"panic", "state-malformed"} end
local now = now_ms()
if not valid_state_identity(KEYS[2], ARGV[1], ARGV[3]) then return {"panic", "state-malformed"} end
if redis.call("HGET", KEYS[2], "state") ~= "claimed" or redis.call("HGET", KEYS[2], "token") ~= ARGV[2]
...
if completed == 5 then return {"exhausted", "5", ARGV[2], ARGV[3], tostring(deadline)} end
local attempt = completed + 1
redis.call("HSET", KEYS[2], "state", "in-flight", "attempt", tostring(attempt))
return {"invoke", tostring(attempt), ARGV[2], ARGV[3], tostring(deadline)}
