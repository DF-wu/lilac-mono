...
Stage 6 separates persistence compatibility from store policy and expected transaction failure from the
SQLite driver's exception contract. Production registrations remain `advisory` until each named symbol
has landed with its compatibility and atomicity tests. Advisory registrations are integrity-checked but
are not resolved or enforced against production source.

## Persisted Codecs

Each `persistedCodecs` entry names one exact callable, its persisted input parameter, one exact fixture
catalog, and its complete success provenance. Activation requires all of the following:
...
`persistedStoreConsumers` links store policy to the codecs it must invoke. Once enforced, a consumer must
call every declared codec, be linked in `operationalResultApis`, and be owned by a zero-baseline scope.
...
Panic classifier, and SQLite driver classifier. An enforced adapter must:
...
`sqliteTransactionConsumers` must invoke their exact adapter and link to `operationalResultApis`. Once a
consumer is enforced, `lilac/no-direct-sqlite-transaction` prohibits direct `.transaction()` and SQL
...
3. Add an exact symbol zero-baseline scope for each consumer. That scope also owns nested callbacks but
   not sibling names with a common prefix.
4. Remove semantic and syntax baseline debt from the scope.
5. Change only the landed registrations from `advisory` to `enforced`.
6. Activate `architecture/no-result-err-in-sqlite-callback` for the adapter module and migrated consumer
   modules.
7. Run architecture tests, lint-rule tests, architecture and lint-plugin typechecks, the architecture
