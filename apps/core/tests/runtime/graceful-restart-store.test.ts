...
describe("graceful restart persisted codec", () => {
  it("roundtrips Telegram relay refs and authenticated origin evidence", () => {
    const snapshot = buildSnapshot();
    const active = snapshot.agent[0];
    const relay = snapshot.relays[0];
    if (!active || !relay) throw new Error("Expected active Telegram fixture inputs");
    const requestId = "telegram:-100123:42:77";
    const sessionId = "-100123:42";
    const messageRef = { platform: "telegram" as const, channelId: sessionId, messageId: "77" };
    const telegramSnapshot: GracefulRestartSnapshotInput = {
      ...snapshot,
      agent: [
        {
          ...active,
          ...
          identity: {
            ...
            projection: {
              ...
              source: "external",
              platform: "telegram",
              sessionRef: { platform: "telegram", channelId: sessionId },
              messageRef,
              authenticatedOrigin: {
                platform: "telegram",
                userId: "9001",
      ...
      relays: [
        {
          ...
          platform: "telegram",
          replyTo: messageRef,
          createdOutputRefs: [{ platform: "telegram", channelId: sessionId, messageId: "78" }],
        },
      ],
    };
    ...
    const decoded = decodeGracefulRestartSnapshot({
      status: "completed",
      payload_json: SuperJSON.stringify(telegramSnapshot),
    });

    if (decoded.status === "error") throw decoded.error;
    expect(decoded.value.value?.relays[0]).toMatchObject({
      platform: "telegram",
      requestClient: "telegram",
      replyTo: messageRef,
    });
    expect(decoded.value.value?.agent[0]?.identity).toMatchObject({
      ...
      projection: {
        ...
        sessionRef: { platform: "telegram", channelId: sessionId },
        messageRef,
        authenticatedOrigin: { platform: "telegram", messageRef },
      },
    });
  });
