...
import {
  ...
  type SurfaceRelayPolicy,
  type SurfaceRelaySnapshotFor,
} from "../../../src/surface/runtime-descriptor";
import { SurfaceRefInvalid } from "../../../src/surface/protocol";
import {
...
class FakeOutputStream {
  ...
  async finish(): Promise<SurfaceOperationResult<SurfaceOutputResult>> {
    this.finished = true;
    const last: MsgRef = { platform: this.platform, channelId: "chan", messageId: "m_out" };
...
function createTelegramRelayPolicy(): SurfaceRelayPolicy<"telegram"> {
  return {
    refs: {
      ...
      decodeReanchorTarget: ({ ref, expectedSessionId }) => {
        if (ref.platform !== "telegram" || ref.channelId !== expectedSessionId) {
          return Result.err(
            new SurfaceRefInvalid({
              ...
              expectedSessionId,
              message: "Telegram relay reference does not match the selected session",
            }),
          );
        }
        return Result.ok({
...
type TestBridgeParams = Omit<Parameters<typeof bridgeBusToAdapterImpl>[0], "policy">;

function bridgeBusToAdapter<P extends "discord" | "github" | "telegram">(
  params: TestBridgeParams & { readonly platform: P },
): Promise<SurfaceRelayHandle<P>>;
async function bridgeBusToAdapter(params: TestBridgeParams): Promise<unknown> {
  ...
  if (params.platform === "telegram") {
    return bridgeBusToAdapterImpl({
      ...params,
      platform: "telegram",
      policy: createTelegramRelayPolicy(),
    });
  }
  return bridgeBusToAdapterImpl({
    ...params,
...
async function applyAndActivateRelayRecovery<P extends "discord" | "github" | "telegram">(
...
describe("bridgeBusToAdapter", () => {
  ...
  it("restores Telegram reasoning status from snapshot", async () => {
    ...
    expect(adapter.stream?.hydratedParts).toContainEqual({
