...
class Parser {
  ...
  private parseEnvironment(): MathNode {
    ...
    const environment = unstarredEnvironment as MatrixEnvironment
    if (!matrixEnvironments.has(environment)) {
      if (this.strict) this.fail(`Unsupported environment ${environment}`)
    ...
    this.fail(`Missing \\end{${rawEnvironment}}`)
  }
